...
[float]
=== true
`true` é frequentemente dito ser definido como 1, o que está correto, porém true possui uma definição mais ampla. Qualquer inteiro que não seja zero é `true`, em um sentido booleano. Então -1, 2 e -200 são todos definidos como true, também, em um sentido booleano.
[%hardbreaks]
